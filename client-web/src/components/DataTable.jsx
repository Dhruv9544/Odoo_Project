...
function Datatable({
    ...
    const convertDate = (htmlContent) => {
        // Use a DOM element to convert HTML to plain text
        const date = new Date(htmlContent);
                        ...
                        if (ele.field == "startDate") {
                            return (
                                <Column
                                    ...
                                    header={ele.header}
                                    body={(rowData) =>
                                        convertDate(rowData.startDate)
                                    }
                                />
                            );
                        } else if (ele.field == "endDate") {
                            return (
                                <Column
                                    key={"datatable" + ind}
                                    headerStyle={headerStyle}
...
function ActionComponent({ action, data }) {
    return (
        <div
            className={`flex ${
                action.length === 1 ? "justify-center" : "justify-evenly"
            }  items-center`}
        >
            {action.map((ele, ind) => {
                return (
